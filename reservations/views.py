    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and form2.is_valid():
            ...
            if (check_out >= check_in):
                ...
                busy_hotel = RestrictionHotels.objects.filter(Q(id_hotel=id_room.id_hotel.id) & (
                    Q(date_on__range=(check_in, check_out)) | Q(date_on__range=(check_in, check_out))))
                if busy_room or busy_hotel:
                    messages.error(
                        request, "Habitación ocupada en el espacio de tiempo seleccionado!")
                else:
        ...
        if form.is_valid():
            ...
            if critery == '1':
                ...
                day = datetime.datetime.strptime(
                    data['day'], '%m/%d/%Y').strftime('%Y-%m-%d')
                reservations = Reservations.objects.filter(
                    Q(id_room__in=rooms) & (Q(check_in=day) | Q(check_out=day)))
            elif critery == '2':
                #weak
                ...
                start = datetime.datetime.strptime(
                    start, '%m/%d/%Y').strftime('%Y-%m-%d')
                end = datetime.datetime.strptime(
                    end, '%m/%d/%Y').strftime('%Y-%m-%d')
                reservations = Reservations.objects.filter(Q(id_room__in=rooms) & (
                    Q(check_in__range=(start, end)) | Q(check_out__range=(start, end))))
            elif critery == '3':
                month = int(data['month'])
                reservations = Reservations.objects.filter(Q(id_room__in=rooms) & (
                    Q(check_in__month=month) | Q(check_out__month=month)))
            if reservations:
                sweetify.success(self.request, 'Búsqueda Exitosa.')
            else:
                sweetify.error(
                    self.request, 'Ningún resultado para el criterio seleccionado')
...
class UpdateReservation(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context = super(UpdateReservation, self).get_context_data(
            *args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        if (check_out >= check_in):
            ...
            if busy_room or busy_hotel:
                messages.error(
                    self.request, "Habitación ocupada en el espacio de tiempo seleccionado!")
                return self.form_invalid(form)
